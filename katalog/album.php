...
<html lang="en">
    ...
    <?php
    include 'koneksi.php';
    $query = mysqli_query($conn, "SELECT p.id, p.nama, p.harga, p.status, p.gambar, k.kategori, g.nama_grup FROM `produk` AS p JOIN kategori AS k ON k.id = p.id_kategori JOIN grub AS g ON g.id = p.id_grub WHERE k.kategori = 'album' ORDER BY p.id ASC;");
    ?>
